...
MLIR Suite integrates the three language servers provided by LLVM:
...
are optional; Tree-sitter highlighting, symbol outlines, bracket matching, and
indentation work without them.

## Server Capabilities

The following matrix reflects
[`llvm/llvm-project@06bf4bfff830`](https://github.com/llvm/llvm-project/commit/06bf4bfff830).
MLIR Suite does not pin or bundle LLVM, so capabilities may differ in other
LLVM builds.
...
Configure each server under `lsp.<server-id>` in Zed's `settings.json`. MLIR
Suite resolves the executable in this order:
