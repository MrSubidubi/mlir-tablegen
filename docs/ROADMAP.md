...
This document tracks the planned direction of **Zed MLIR**. Core language and LSP integration is in place, so remaining work favors focused parser and query maintenance over broad feature expansion. Items are grouped by priority, not by commitment — timing depends on upstream LLVM releases, Zed extension-API surface, concrete user feedback, and available time. Nothing here is a promise.
